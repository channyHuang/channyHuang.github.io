...
## 设计模式
根据目的和用途不同,<font color = red>设计模式可分为创建型(creational)模式、结构型(structural) 模式和行为型(behavioral)模式三种</font>。
* 1.创建型模式  
...
* 原型化方法: 迭代开发
* 面向对象方法: 封装继承多态
  * 面向对象的设计原则: 开闭原则、里氏替换原则（父类）、依赖倒置原则（参数）、组合/聚合复用原则、接口隔离原则（功能分离）、最少知识原则（解耦合）
* 面向服务方法: 操作、服务、业务流程
* 快速应用开发方法(RAD): 业务建模、数据建模、过程建模、应用生成、测试与交互
# 系统设计
## 处理流程设计
## 设计模式
* 创建型模式
  * 实例化过程。工厂模式、单例模式、原型模式、建造着模式等
* 结构型模式
  * 类结构模式和对象结构模式。适配器模式、桥接模式、组合模式、装饰模式、外观模式等
* 行为型模式
   * 类行为模式和对象行为模式。解释器模式、迭代器模式、观察者模式
# 需求工程
## 面向对象分析方法
* 统一建模语言UML
* 用例模型
