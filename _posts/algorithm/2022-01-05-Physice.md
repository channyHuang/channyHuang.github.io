...
| 质量：m = F / a | 转动惯量：单质点 I = m r^2, 刚体 I = \int \rho r^2 dV|
...
* solveGroup (btDiscreteDynamicsWorld::processIsland调用) PGS 方法来处理分析所有的约束
	* solveGroupCacheFriendlySetup 三类nonContact、contact、friction
		* setupSolverFunctions 设置后续求解器的三个函数
			* gResolveSingleConstraintRowGeneric_scalar_reference PGS/SI内部求解单行约束
			* gResolveSingleConstraintRowLowerLimit_scalar_reference
			* gResolveSplitPenetrationImpulse_scalar_reference
		* convertBodies 计算每个body的torque impulse
			* getOrInitSolverBody
				* initSolverBody 设置系数默认值
		* convertJoints 对每个约束，创建Jacobian矩阵，及其它数据，J*v = c + cfm * lambda
